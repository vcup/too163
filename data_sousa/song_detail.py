...
class song_detail_old:
    ...
    def artist_iter(self, i1: int = 0) -> artist:
        """迭代指定索引位置的歌曲的所有歌手信息"""
        return (self.artist(i1, i2) for i2 in range(len(self.path / f'[{i1}]/artists')))

    def all_aritst_iter(self) -> artist_iter:
        """迭代所有请求歌曲的所有歌手信息"""
    ...
    def all_alias_iter(self) -> alias_iter:
        """迭代所有请求歌曲的所有别名"""
        return (a for i1 in range(self.len) for a in self.artist_iter(i1))
